...
- 주제
    ...
    - 작동 확인 : 로그에서 확인 가능, 대시보드에서 시각적 확인

- 특징
    - 현재 프로젝트 디렉토리는 docker 상에 특정 컨테이너와 연동되어 있음
    - 작성한 코드들은 airflow 현재 생태계 내부로 공유됨 -> 인식됨 -> 대시보드에서 보임 : 볼륨 설정
'''
# 1. 필요한 패키지, 모듈 가져오기 
## DAG 클래스 가져오기
...
# 2-1. DAG를 정의한 필요한 파라미터를 외부에서 설정(옵션), DAG 내부에서도 가능
default_args = {
    ...
    # 시나리오 : 작업 실패 => 5분 후 => 1회 재시도 => 실패 => 완료(실패) => 향후 일정에서 성공하더라도 실패 데이터 소급 X, 이번 주기 획득할/처리 등 데이터 포기
}
...
with DAG(
    ...
    schedule_interval = '@daily', # 하루에 한번 00시 00분, cron 표현 가능
    start_date = datetime(2026,2,25), # 현재 시점에서 시작일과의 차이를 고려하여 소급 처리 여부 체크 
                                      # 기본 설정에서 처리 x 설정했음 => 무시됨
    catchup = False, # 과거에 대한 소급 처리 실행 방지 옵션
                     # 해당 조치가 없었다면 현재일 - (2026.2.25) 차이만큼 소급 처리 수행됨
    tags = ['bash', 'basic'] # DAG가 많으면 찾기 힘들 수 있음 -> 검색 -> 관리
) as dag:
    # 3. DAG 세션 오픈
    # 3-1. Operator 구성
    # 오퍼레이터 객체 생성 -> Task가 정의됨 -> 구동 -> Task Instance 생성됨
    # BashOperator를 통해서 작업(bash)을 3개 정의
    t1 = BashOperator(
        # id 구성값 : 영문자, 숫자, 하이픈, 마침표, 언더바만으로 구성
        # airflow의 지휘하에 작동되는 DAG 구동시 실제 할 일을 구성하는 task 구분값
        # 작동 확인 => 로그
        task_id = 'date-print',
    ...
    # t1 실행 -> t2 실행 -> t3 실행
    # loop x, 방향성 가짐
    # t1 실행이 성공해야만 t2 실행.. 이후 같은 패턴 반복 -> 의존성 존재
    # 대시 보드에서 graph 메뉴에서 노드 형태로 확인 가능
    t1 >> t2 >> t3
    pass
